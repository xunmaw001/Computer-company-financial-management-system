...
export default {
  data() {
	...
	shenfenzheng : false,
      ...
      rules: {
          guanligonghao: [
                { required: true, message: '管理工号不能为空', trigger: 'blur' },
          ],
          mima: [
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yuangongguanli/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuangongguanliCrossAddOrUpdateFlag = false;
                  this.parent.search();
